                ...
                if objects[index][ObjectProp.ObjType] != ObjectType.Absent:
                    ...
                    for jndex in range(0, ObjectType.ObjArrayTotal):
                        if objects[jndex][ObjectProp.ObjType] != ObjectType.Absent and index != jndex:
                            ...
                            self.angle_between_radius = np.fabs((objects[jndex][ObjectProp.Dir] - np.degrees(np.arctan(self.diff_vector[1]/self.diff_vector[0])))%360)
                            self.angle_between_radius = 360 - self.angle_between_radius if self.angle_between_radius > 180 else self.angle_between_radius
                            self.angle_between_objects = np.fabs((objects[jndex][ObjectProp.Dir] - objects[index][ObjectProp.Dir])%360)
